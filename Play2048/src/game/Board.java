...
public class Board {
    ...
    public void addRandomTile() {
        int random = StdRandom.uniform(0, (int)openSpaces.size());
        BoardSpot spot = openSpaces.get(random);
        int r = spot.getRow();
        int c = spot.getCol();
    ...
    public void flipRows() {
        ...
        for (int i = 0; i < gameBoard.length; i++){
            int leftSide = 0;
            int rightSide = gameBoard[i].length - 1;
            while (leftSide <= rightSide){
                int temp = gameBoard[i][leftSide];
                gameBoard[i][leftSide] = gameBoard[i][rightSide];
                gameBoard[i][rightSide] = temp;
                leftSide++;
                rightSide--;
                score++;
            }
